...
// -----------------------------------------------------------------------------
  ...
  $CONTENT = '
    <html>
      ...
      <body>
        <p><strong style="width: 80px;">Name: </strong>' . $_POST['fname'] . ' ' . $_POST['lname'] . '</p>
  ...
  if($_SERVER['REQUEST_METHOD'] != 'POST') {
    ...
    if (empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest') {
      ...
      if (empty($EMAIL) || empty($NAME) || empty($SUBJECT) || empty($CONTENT)) {
        ...
        $mail->CharSet = 'UTF-8';
        $mail->setFrom($form_email, $form_fname . ' ' . $form_lname);
        $mail->addReplyTo($form_email, $form_fname . ' ' . $form_lname);
